...
Due to the limited amount of resources available to the project, generating a dataset with sufficient scope and size is challenging. Since the performance of machine learning models is usually directly tied to the size and quality of the available dataset (XXX citation needed), advanced methods will have to be employed to artificially increase the amount of distinct data samples.

\citet{mikolajczyk_data_2018} make the distinction between white-box and black-box data augmentation methods. While the latter use a form of deep learning (Generative Adversarial Networks) to synthesise new samples, white-box methods involve more traditional techniques, such as affine transformations.

Although this project focuses primarily on white-box data augmentation, the implementation of a visible light to LWIR autencoder is discussed in Section \ref{autoencoder_implementation}. This autoencoder could be used to generate new LWIR data samples from visible light images retrieved from other sources, such as existing datasets.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}[h!]{0.4\textwidth}
    \includegraphics[width=\textwidth]{images/registration/before_registration.png}
    \caption{Before registration. The LWIR and VIS images are visibly misaligned.}
  \end{subfigure}
  \begin{subfigure}[h!]{0.4\textwidth}
    \includegraphics[width=\textwidth]{images/registration/after_registration.png}
...
A deep convolutional autoencoder was created to reconstruct the LWIR signature of objects from the corresponding visible light image. 

As shown in Figure \ref{fig:autoencoder_architecture}, the chosen architecture for the encoder consists of four blocks. Each block has a convolutional layer, followed by a leaky ReLU activation function, and a subsequent max pooling layer for downsampling. The $320 \times 240 \times 1$ input images are thus processed and downsampled to a $20 \times 15 \times 32$ tensor containing a compact, latent representation of the features in the original image.

The encoder then uses four deconvolutional layers to attempt a reconstruction the LWIR image from the latent features output by the encoder.
