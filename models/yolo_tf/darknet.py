...
def darknet(inputs):
    ...
    with tf.compat.v1.variable_scope("darknet"):
        ...
        for i in range(1):
            inputs = layers.residual_block(f"residual_{i}", inputs, num_filters=32)
        
        inputs = layers.conv_layer(name="darknet_conv_2", inputs=inputs, filters=128, kernel_size=3, downsample=True)
        
        
        for i in range(2):
            inputs = layers.residual_block(f"residual_{i + 1}", inputs, num_filters=64)
        
        inputs = layers.conv_layer(name="darknet_conv_3", inputs=inputs, filters=256, kernel_size=3, downsample=True)
        ...
        for i in range(4):
            inputs = layers.residual_block(f"residual_{i + 19}", inputs, num_filters=512)
